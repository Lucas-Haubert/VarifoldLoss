# Overview of the class Exp_Long_Term_Forecast

# Basic initialization of the class
# __init__, _build_model, _get_data, _get_data_structural, _select_optimizer, _select_criterion

# Training, test, prediction
# train, test, test_on_vali, test_structural, predict

# Visualization
# cumulative_computing_loss_metrics, plot_losses_and_metrics, plot_gains_wrt_epochs, plot_batches_without_prediction
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def cumulative_computing_loss_metrics(self, dataloader, criterion):
        ...
        metrics_over_batches = {metric: [] for metric in self.list_of_metrics}
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(dataloader):
                ...
                metrics_current_batch = compute_metrics(pred, true, self.list_of_metrics)
        ...
        metrics_current_epoch = compute_metrics(preds, trues, self.list_of_metrics) 
    ...
    def plot_losses_and_metrics(self, metric_name, setting):
        ...
        plt.title(f"Model: {self.args.model}, Loss: {self.args.loss}, Metric: {metric_name}, W: {self.args.seq_len}, H: {self.args.pred_len}, Dataset: {self.args.data_path.split('.')[0]}", pad=20)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            if self.args.plot_metrics_and_gains == 1:

                start_time_metrics = time.time()

                train_loss = np.average(train_loss)
                vali_loss, vali_metrics_current_epoch = self.cumulative_computing_loss_metrics(vali_loader, criterion)
                test_loss, test_metrics_current_epoch = self.cumulative_computing_loss_metrics(test_loader, criterion)
            ...
            elif self.args.plot_metrics_and_gains == 0:
                train_loss = np.average(train_loss)
    ...
    def test(self, setting, test=0):
        ...
        if self.args.bool_plot_examples_without_forecasts == 1:
            self.plot_batches_without_prediction(test_data, test_loader, dataset_name, setting)
        ...
        results = compute_metrics(preds, trues, self.list_of_metrics)
        ...
        file_path = os.path.join(folder_path, f"txt_metrics_per_iteration{setting}.txt")
    ...
    def test_on_vali(self, setting, test=0):
        ...
        file_path = os.path.join(folder_path, f"txt_metrics_on_vali_per_iteration{setting}.txt")
        with open(file_path, 'a') as f:
            f.write(setting + "  \n")
            f.write(formatted_metrics + '\n')
            f.write('\n')
    ...
    def test_structural(self, setting, test=0):
        ...
        dataset_name_raw = self.args.data_path.split('.')[0]
        dataset_name_structural = self.args.structural_data_path.split('.')[0]
        
        if self.args.bool_plot_examples_without_forecasts == 1:
            self.plot_batches_without_prediction(test_data, test_loader, dataset_name_raw, setting)
            self.plot_batches_without_prediction(structural_test_data, structural_test_loader, dataset_name_structural, setting)
